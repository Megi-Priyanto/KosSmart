...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
            ...
            @else
                ...
                <div class="aspect-video bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center">
                    <div class="text-center">
                        ...
                        <p class="text-white font-medium">Foto belum tersedia</p>
                    </div>
                </div>
            @endif
        </div>
        ...
        <div class="bg-gradient-to-br from-yellow-500 to-amber-600 rounded-xl shadow-lg p-6 mb-8 text-white">
            ...
            <div class="space-y-3">
                <div>
                    <p class="text-2xl font-bold text-white mb-1">{{ $kosInfo->name }}</p>
                    <p class="text-white">{{ $kosInfo->full_address }}</p>
                </div>

                <div class="grid grid-cols-2 gap-4 pt-3">
                    <div>
                        <p class="text-sm text-white">Telepon</p>
                        <p class="font-semibold text-white">{{ $kosInfo->phone }}</p>
                    </div>

                    @if($kosInfo->whatsapp)
                    <div>
                        <p class="text-sm text-white">WhatsApp</p>
                        <p class="font-semibold text-white">{{ $kosInfo->whatsapp }}</p>
                    </div>
                    @endif
                </div>
                ...
                @if(!empty($rules))
                    <div class="pt-3 border-t border-yellow-200">
                        <p class="text-sm font-medium text-white mb-2">Peraturan Kos:</p>
                        <ul class="space-y-1">
                            @foreach(array_slice($rules, 0, 10) as $rule)
                                <li class="flex items-start text-sm text-white">
                                    <svg class="w-4 h-4 text-white mr-2 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
