...
<!-- Breadcrumb -->
<nav class="mb-4 flex items-center text-xs sm:text-sm text-slate-400">
    <a href="{{ route('user.dashboard') }}" class="hover:text-amber-600 transition">
...
<div class="bg-white rounded-xl shadow-md border border-yellow-200 p-4 sm:p-5 mb-6">
    <form method="GET" action="{{ route('user.rooms.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div x-data="{ open:false, selected:'{{ request('type') ? ucfirst(request('type')) : 'Semua Tipe' }}' }" class="relative">
            <label class="block text-sm font-semibold text-amber-600 mb-2">
                Tipe Kamar
            </label>
            ...
            <div class="p-3 sm:p-4 flex flex-col flex-1">
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    ...
                    @foreach(array_slice($facilities ?? [], 0, 3) as $facility)
                        <span class="px-2 py-1 bg-amber-50 text-amber-600 text-xs rounded-full">
                            {{ $facility }}
                        </span>
                    @endforeach
                </div>
...
<section class="mt-12 pt-10 border-t border-[#e2e8f0]">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h2 class="text-xl font-bold text-slate-800">Rating & Ulasan</h2>
            <p class="text-sm text-slate-400">{{ $totalUlasan }} ulasan dari penghuni</p>
        </div>
    </div>

    {{-- Ringkasan rating --}}
    <div class="bg-white border border-[#e2e8f0] rounded-2xl shadow-none p-6 mb-6"
        ...
        <div class="text-center" style="min-width:130px;">
            ...
            <div class="text-sm text-slate-500">dari {{ $totalUlasan }} ulasan</div>
        </div>
        ...
        <div style="display:flex;flex-direction:column;gap:0.55rem;">
            ...
            <div style="display:flex;align-items:center;gap:0.65rem;">
                <span class="text-xs text-slate-500" style="width:14px;text-align:right;flex-shrink:0;">{{ $i }}</span>
                ...
                <span class="text-xs text-slate-500" style="width:28px;text-align:right;flex-shrink:0;">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div x-data="{
            ...
            <div class="ulasan-footer">
                ...
                <div>
                    <p class="text-xs font-semibold text-slate-600" style="line-height:1.2;">
...
{{-- Belum ada ulasan --}}
@if(isset($tempatKos))
<section class="mt-10 p-6 bg-[#f8fafc] border border-[#e2e8f0] rounded-2xl text-center">
    <svg class="w-10 h-10 mx-auto mb-3 text-slate-500" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
    </svg>
    <p class="text-sm text-slate-400">Belum ada ulasan untuk kos ini.</p>
</section>
@endif
@endif
...
@else
    <!-- Empty State -->
    <div class="text-center py-16 bg-white rounded-xl shadow-none border border-[#e2e8f0]">
        <svg class="w-16 h-16 mx-auto text-slate-500 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path>
        </svg>
        <h3 class="text-xl font-bold text-slate-600 mb-2">Belum Ada Kamar</h3>
