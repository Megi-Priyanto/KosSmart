...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Status Pembatalan Booking</h1>
            <p class="text-sm text-slate-500 mt-1">Pantau proses pengembalian dana DP Anda</p>
        </div>
    ...
    <!-- Status Card -->
    <div class="bg-white rounded-xl shadow-lg border border-[#e2e8f0] p-6">
        
        <!-- Status Badge -->
        <div class="mb-6">
            <h3 class="text-lg font-bold text-slate-800 mb-3">Status Saat Ini</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="space-y-4">
                <h4 class="font-bold text-slate-800 pb-2 border-b">Informasi Booking</h4>
                <div>
                    <p class="text-sm text-slate-500">Kamar</p>
                    <p class="font-semibold text-slate-800">{{ $cancelBooking->rent->room->room_number }}</p>
                </div>
                <div>
                    <p class="text-sm text-slate-500">DP yang Dibayar</p>
                    <p class="text-xl font-bold text-amber-600">Rp {{ number_format($cancelBooking->rent->deposit_paid, 0, ',', '.') }}</p>
                </div>
                <div>
                    <p class="text-sm text-slate-500">Tanggal Pengajuan</p>
                    <p class="font-semibold text-slate-800">{{ $cancelBooking->created_at->format('d M Y, H:i') }}</p>
                </div>
            </div>

            <div class="space-y-4">
                <h4 class="font-bold text-slate-800 pb-2 border-b">Rekening Pengembalian</h4>
                <div>
                    <p class="text-sm text-slate-500">Bank</p>
                    <p class="font-semibold text-slate-800">{{ $cancelBooking->bank_name }}</p>
                </div>
                <div>
                    <p class="text-sm text-slate-500">Nomor Rekening</p>
                    <p class="font-semibold text-slate-800">{{ $cancelBooking->account_number }}</p>
                </div>
                <div>
                    <p class="text-sm text-slate-500">Atas Nama</p>
                    <p class="font-semibold text-slate-800">{{ $cancelBooking->account_holder_name }}</p>
                </div>
            </div>
        ...
        <div class="mt-6 p-6 bg-green-900/30 border-l-4 border-green-500 rounded-lg">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <p class="text-sm text-green-700">Metode Pengembalian</p>
        ...
        <!-- Pending Info -->
        @if($cancelBooking->status === 'pending')
        <div class="mt-6 p-6 bg-amber-50 border-l-4 border-yellow-500 rounded-lg">
