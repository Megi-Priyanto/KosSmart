...
<div class="space-y-6" x-data="bookingForm()">
    ...
    <div class="flex items-center justify-between mb-2">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Booking Kamar {{ $room->room_number }}</h1>
            <p class="text-sm text-slate-500 mt-1">Lengkapi formulir di bawah ini untuk melakukan booking</p>
        </div>
    ...
    <form action="{{ route('user.booking.store', $room) }}" 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-none border border-[#e2e8f0] overflow-hidden">
                    
                    <!-- Form Header -->
                    <div class="p-6 border-b border-[#e2e8f0] bg-[#f8fafc]">
                        <h2 class="text-xl font-bold text-gray-900">Form Booking</h2>
                        <p class="text-sm text-slate-500 mt-1">Lengkapi data booking dan upload bukti pembayaran DP</p>
                    </div>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="date" 
                                   ...
                                   class="w-full px-4 py-3 border border-[#e2e8f0] rounded-lg focus:ring-2 focus:ring-yellow-500"
                                   required>
                            @error('start_date')
                        ...
                        <div>
                            ...
                            <div x-show="selectedMethod" x-transition class="mt-4">
                                ...
                                <div x-show="selectedMethod !== 'qris'" class="p-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl">
                                    <p class="font-semibold text-slate-800 mb-3">Transfer ke:</p>
                                    <div class="space-y-2">
                                        <div class="flex justify-between">
                                            <span class="text-slate-500" x-text="selectedMethod === 'manual_transfer' ? 'Bank:' : 'E-Wallet:'"></span>
                                            <span class="font-semibold text-slate-800" x-text="getSubMethodLabel()"></span>
                                        </div>
                                        <div class="flex justify-between">
                                        ...
                                        <div class="flex justify-between">
                                            ...
                                            <span class="font-semibold text-slate-800" x-text="getAccountHolder()"></span>
                                        </div>
                                        <div class="flex justify-between pt-2 border-t border-yellow-200">
                                            <span class="text-slate-500">Jumlah DP:</span>
                                            <span class="text-xl font-extrabold text-amber-600">Rp {{ number_format($depositAmount, 0, ',', '.') }}</span>
                                        </div>
                                    </div>
                                </div>
                                
                                <!-- QRIS -->
                                <div x-show="selectedMethod === 'qris'" class="bg-amber-50 rounded-xl border border-yellow-200 p-6 rounded-xl text-center">
                                    <p class="font-semibold text-slate-800 mb-4">Scan QR Code untuk Pembayaran</p>
                                    <img src="{{ asset('storage/qris/dana.jpeg') }}" 
                                         alt="QRIS" 
                                         class="w-64 h-64 mx-auto border-4 border-white shadow-lg rounded-lg mb-4">
                                    <p class="text-lg font-bold text-amber-600 mb-2">Rp {{ number_format($depositAmount, 0, ',', '.') }}</p>
                                    <p class="text-sm text-slate-500">Scan dengan aplikasi pembayaran favorit Anda</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-show="selectedMethod" x-transition>
                            ...
                            <input type="file" 
                                   ...
                                   accept="image/*"
                                   @change="previewImage"
                                   class="w-full border border-[#e2e8f0] rounded-lg px-4 py-2"
                                   required>
                            @error('deposit_proof')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                            <p class="text-sm text-slate-400 mt-1">Format: JPG, PNG (Max. 2MB)</p>
                            
                            <div x-show="imagePreview" class="mt-4">
                                <p class="text-sm font-medium text-slate-600 mb-2">Preview:</p>
                                <img :src="imagePreview" class="max-w-md rounded-lg border-2 border-[#e2e8f0]">
                            </div>
                        </div>

                        <!-- Agreement -->
                        <div>
                            <div class="flex items-start p-3 bg-[#f8fafc] rounded-lg">
                                <input type="checkbox" 
                                       name="agreement" 
                                       value="1"
                                       id="agreement"
                                       ...
                                       required>
                                <label for="agreement" class="ml-3 text-sm text-slate-600">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-lg border-2 border-slate-200 p-6 sticky top-6">
                    <h3 class="text-lg font-bold text-slate-800 mb-4">Ringkasan Booking</h3>
                    ...
                    <div class="space-y-3 mb-6">
                        ...
                        <div class="flex justify-between text-sm border-t pt-3">
                            <span class="text-slate-500">DP (50%)</span>
                            <span class="font-semibold text-amber-600">Rp {{ number_format($depositAmount, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>

                <!-- Bantuan -->
                <div class="bg-amber-50 rounded-xl border border-yellow-200 p-6">
                    ...
                    <div class="space-y-2">
                        <a href="tel:{{ $room->kosInfo->phone }}" 
                           class="flex items-center text-sm text-slate-600 hover:text-amber-600">
                        ...
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $room->kosInfo->whatsapp) }}" 
                           target="_blank"
                           class="flex items-center text-sm text-slate-600 hover:text-green-600">
    ...
    <div x-show="showPaymentModal" 
        ...
        <div @click.away="showPaymentModal = false"
            ...
            <div class="sticky top-0 bg-white border-b p-6 flex justify-between items-center">
                ...
                <button @click="showPaymentModal = false" class="text-slate-500 hover:text-slate-500">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="border-2 border-[#e2e8f0] rounded-xl p-4">
                    <h4 class="font-bold text-lg mb-3">{{ $data['label'] }}</h4>
                    
                    @if($method === 'qris')
                    <button type="button"
                            @click="selectPayment('qris', 'qris')"
                            class="w-full p-4 border-2 rounded-lg hover:border-yellow-500 hover:bg-amber-50 transition-all text-left"
                            :class="selectedMethod === 'qris' ? 'border-yellow-500 bg-amber-50' : 'border-[#e2e8f0]'">
                        <div class="flex items-center justify-between">
                            ...
                            <svg x-show="selectedMethod === 'qris'" class="w-6 h-6 text-amber-600" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <button type="button"
                                ...
                                :class="selectedSubMethod === '{{ $key }}' ? 'border-amber-500 bg-amber-50' : 'border-[#e2e8f0]'">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="font-semibold">{{ $option['name'] }}</p>
                                    <p class="text-xs text-slate-400">{{ $option['account'] }}</p>
                                </div>
