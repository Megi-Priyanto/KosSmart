...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between mb-2">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Status Booking Anda</h1>
            <p class="text-sm text-slate-500 mt-1">Pantau status booking kamar Anda di sini</p>
        </div>
        <a href="{{ route('user.status.index') }}" 
           class="px-4 py-2 bg-gradient-to-r from-yellow-500 to-orange-600 text-white font-semibold rounded-lg hover:from-yellow-600 hover:to-orange-700 transition-all shadow-lg flex items-center justify-center">
            Kembali ke Status History
        </a>
    </div>
    
    <!-- Status Timeline -->
    <div class="bg-white rounded-xl shadow-lg border border-[#e2e8f0] p-6 mb-6">
        <div class="mb-6">
            <h3 class="text-lg font-bold text-slate-800 mb-4">Status Saat Ini</h3>
            ...
            <div class="inline-flex items-center space-x-2 px-4 py-2 rounded-full text-lg font-semibold
                ...
                @elseif($rent->status == 'cancelled') bg-red-900/50 text-red-900 ring-1 ring-red-400
                @else bg-white text-slate-800
                @endif">
        ...
        <div class="relative">
            ...
            <div class="space-y-6">
                ...
                <div class="relative flex items-start pl-12">
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-slate-800">Booking Diterima</h4>
                        <p class="text-sm text-slate-500">{{ $rent->created_at->format('d M Y, H:i') }} WIB</p>
                        <p class="text-xs text-slate-400 mt-1">Booking Anda telah kami terima dan sedang diproses</p>
                    </div>
                </div>
                ...
                <div class="relative flex items-start pl-12">
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-slate-800">Pembayaran Lunas</h4>
                        <p class="text-sm text-slate-500">
                            @if($rent->status == 'active')
                                Selesai
                            @else
                        ...
                        <p class="text-xs text-slate-400 mt-1">Pembayaran sisa dan mulai menempati kamar</p>
                    </div>
                </div>
                ...
                <div class="relative flex items-start pl-12">
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-slate-800">Mulai Menempati</h4>
                        <p class="text-sm text-slate-500">{{ $rent->start_date->format('d M Y') }}</p>
                        <p class="text-xs text-slate-400 mt-1">Anda dapat mulai menempati kamar pada tanggal ini</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <!-- Info Kamar -->
        <div class="bg-white rounded-xl shadow-none border border-[#e2e8f0] p-6">
            ...
            <div class="space-y-3">
                <div class="flex justify-between">
                    ...
                    <span class="font-semibold text-slate-800">{{ $rent->room->room_number }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-slate-500">Lantai</span>
                    <span class="font-semibold text-slate-800">{{ $rent->room->floor }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-slate-500">Tipe</span>
                    <span class="font-semibold text-slate-800">{{ ucfirst($rent->room->type) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-slate-500">Ukuran</span>
                    <span class="font-semibold text-slate-800">{{ $rent->room->size }} m²</span>
                </div>
            </div>
            
            <div class="mt-4 pt-4 border-t border-[#e2e8f0]">
                <a href="{{ route('user.rooms.show', $rent->room->id) }}" 
                   class="text-amber-600 hover:text-yellow-500 text-sm font-medium flex items-center">
                    Lihat Detail Kamar
        ...
        <div class="bg-white rounded-xl shadow-none border border-[#e2e8f0] p-6">
            ...
            <div class="space-y-3 mb-4">
                <div class="flex justify-between">
                    <span class="text-slate-500">Harga Sewa/Bulan</span>
                    <span class="font-semibold text-slate-800">Rp {{ number_format($rent->monthly_rent, 0, ',', '.') }}</span>
                </div>
                <div class="flex justify-between">
                ...
                <div class="flex justify-between pt-3 border-t border-orange-200 bg-orange-50 px-3 py-2 rounded-lg">
                    ...
                    <span class="font-semibold text-amber-600">Rp {{ number_format($rent->monthly_rent - $rent->deposit_paid, 0, ',', '.') }}</span>
                </div>
            </div>
            ...
            <div class="mt-4 pt-4 border-t border-[#e2e8f0]">
                <p class="text-sm font-medium text-slate-600 mb-2">Bukti Transfer DP:</p>
                @php
                    $imagePath = str_replace('Bukti DP: ', '', $rent->notes);
                @endphp
                <img src="{{ asset('storage/' . $imagePath) }}" 
                     alt="Bukti Transfer"
                     class="w-full rounded-lg border border-[#e2e8f0] cursor-pointer hover:opacity-90"
            ...
            @if($rent->status == 'pending')
            <div class="mt-4 p-3 bg-amber-50 border border-yellow-200 rounded-lg">
                <p class="text-sm text-amber-700">
                    Sisa pembayaran akan diinformasikan setelah booking disetujui
                </p>
            </div>
            @elseif($rent->status == 'active')
            <div class="mt-4 p-3 bg-green-900/30 border border-green-200 rounded-lg">
    ...
    <!-- Info Tanggal -->
    <div class="bg-gradient-to-br from-yellow-50 to-amber-50 rounded-xl border border-yellow-200 p-6 mb-6">
        <h3 class="text-lg font-bold text-slate-800 mb-4">Jadwal</h3>
        
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <p class="text-sm text-slate-500 mb-1">Tanggal Booking</p>
                <p class="font-semibold text-slate-800">{{ $rent->created_at->format('d M Y') }}</p>
                <p class="text-xs text-slate-400">{{ $rent->created_at->diffForHumans() }}</p>
            </div>
            <div>
                <p class="text-sm text-slate-500 mb-1">Mulai Sewa</p>
                <p class="font-semibold text-slate-800">{{ $rent->start_date->format('d M Y') }}</p>
            ...
            <div>
                <p class="text-sm text-slate-500 mb-1">Status Sewa</p>
    ...
    @if($rent->status == 'pending')
    <div class="bg-yellow-900/30 border-l-4 border-yellow-500 p-6 rounded-lg mb-6 ring-1 ring-yellow-200">
        <div class="flex items-start">
            <svg class="w-6 h-6 text-yellow-500 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            <div>
                <h4 class="font-bold text-amber-700 mb-2">Menunggu Konfirmasi Admin</h4>
    ...
    <div class="bg-green-100 border-l-4 border-green-500 p-6 rounded-lg mb-6 ring-1 ring-green-200">
        <div class="flex items-start">
            ...
            <div>
                <h4 class="font-bold text-green-300 mb-2">Booking Aktif</h4>
    ...
    @elseif($rent->status == 'cancelled')
    <div class="bg-red-900/40 border-l-4 border-red-500 p-6 rounded-lg mb-6 ring-1 ring-red-200">
        <div class="flex items-start">
            <svg class="w-6 h-6 text-red-500 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            <div>
                <h4 class="font-bold text-red-300 mb-2">Booking Dibatalkan</h4>
    ...
    <!-- Contact Admin -->
    <div class="bg-white rounded-xl shadow-none border border-[#e2e8f0] p-6">
        <h3 class="text-lg font-bold text-slate-800 mb-4">Hubungi Admin</h3>
        
        @if($rent->room->kosInfo)
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <a href="tel:{{ $rent->room->kosInfo->phone }}" 
               class="flex min-w-0 items-center space-x-3 p-4 border border-[#e2e8f0] rounded-lg hover:bg-[#f8fafc] transition-colors">
                ...
                <div class="min-w-0">
                    ...
                    <p class="font-semibold text-slate-800 truncate">
                        {{ $rent->room->kosInfo->phone }}
                    </p>
                </div>
            </a>
            
            @if($rent->room->kosInfo->whatsapp)
            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $rent->room->kosInfo->whatsapp) }}"
               target="_blank"
               class="flex min-w-0 items-center space-x-3 p-4 border border-[#e2e8f0] rounded-lg hover:bg-[#f8fafc] transition-colors">
