...
<div class="mb-6 bg-[rgba(245,158,11,0.08)] border border-[rgba(245,158,11,0.35)] rounded-2xl p-5 flex items-start gap-4 shadow-sm">
    <div class="flex-shrink-0 w-12 h-12 bg-[rgba(245,158,11,0.15)] rounded-xl flex items-center justify-center">
    ...
    <a href="{{ route('user.ulasan.create', $billingPerluDiulas) }}"
       class="flex-shrink-0 px-4 py-2 bg-[rgba(245,158,11,0.08)]0 hover:bg-yellow-600 text-white text-sm font-bold rounded-xl transition">
        Tulis Ulasan
    </a>
</div>
@endif
...
<div x-data="{ showCancelModal: false }">
    <div class="mb-6 bg-[rgba(245,158,11,0.08)] border-l-4 border-yellow-500 p-6 rounded-lg">
        <div class="flex items-start justify-between">
            ...
            @else
            <span class="ml-4 px-4 py-2 bg-[rgba(251,146,60,0.15)] text-orange-400 text-sm font-semibold rounded-lg whitespace-nowrap">
    ...
    <div x-show="showCancelModal" x-cloak
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <button @click="showCancelModal = false" class="text-gray-600 hover:text-gray-500">
            ...
            <form action="{{ route('user.booking.cancel.store', $pendingRent) }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-600 mb-2">Nama Bank</label>
                    <select name="bank_name" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-yellow-500">
                ...
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-600 mb-2">Nomor Rekening</label>
                    <input type="text" name="account_number" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-yellow-500" placeholder="1234567890">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-600 mb-2">Nama Pemilik Rekening</label>
                    <input type="text" name="account_holder_name" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-yellow-500" placeholder="JOHN DOE">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-600 mb-2">Alasan Pembatalan (Opsional)</label>
                    <textarea name="cancel_reason" rows="3" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-yellow-500" placeholder="Jelaskan alasan Anda..."></textarea>
                </div>
                <div class="bg-[rgba(245,158,11,0.08)] border border-[rgba(245,158,11,0.3)] rounded-lg p-3 mb-4">
                ...
                <div class="flex gap-3">
                    <button type="button" @click="showCancelModal = false" class="flex-1 px-4 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 transition">Batal</button>
                    <button type="submit" class="flex-1 px-4 py-2 bg-[rgba(248,113,113,0.08)]0 text-white rounded-lg hover:bg-red-600 transition font-semibold">Ajukan Pembatalan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('user.dashboard') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}"
                   ...
                   class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent">
        </div>
        <select name="kota" class="px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent">
        ...
        <div class="flex gap-2">
            ...
            @if(request()->hasAny(['search', 'kota']))
                <a href="{{ route('user.dashboard') }}" class="px-4 py-2.5 border border-gray-200 rounded-lg hover:bg-gray-50 transition">Reset</a>
            @endif
        </div>
    </form>
</div>
...
@if($tempatKos->count() > 0)
    <div class="mb-6">
        ...
        <p class="text-gray-500">{{ $tempatKos->total() }} tempat kos tersedia</p>
    </div>
        ...
        @foreach($tempatKos as $kos)
            <a href="{{ route('user.rooms.index', ['tempat_kos_id' => $kos->id]) }}"
               class="group bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg hover:border-yellow-400 transition-all duration-300 flex flex-col">
                ...
                <!-- Gambar -->
                <div class="relative h-36 sm:h-40 bg-[#374151] overflow-hidden flex-shrink-0">
                    @if($kos->logo)
                ...
                <div class="p-3 sm:p-4 flex flex-col flex-grow">
                    <h3 class="font-semibold text-gray-800 text-sm sm:text-base mb-1 group-hover:text-yellow-600 transition line-clamp-1">
                        {{ $kos->nama_kos }}
                    </h3>
                    ...
                    <div class="flex items-start text-xs sm:text-sm text-gray-500 mb-3 min-h-[2.5rem]">
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-gray-200 mb-3">
                        <div class="flex items-center text-sm">
                            ...
                            <span class="font-medium text-gray-600">{{ $kos->total_kamar }} Kamar</span>
                        </div>
                        @if($kos->kamar_tersedia > 0)
                            <span class="text-xs font-medium text-green-600 bg-[rgba(52,211,153,0.08)] px-2 py-1 rounded-full">{{ $kos->kamar_tersedia }} Tersedia</span>
                        @else
                            <span class="text-xs font-medium text-red-600 bg-[rgba(248,113,113,0.08)] px-2 py-1 rounded-full">Penuh</span>
                        @endif
                    </div>
...
@else
    <div class="text-center py-10 sm:py-12">
        ...
        <h3 class="text-base sm:text-lg font-semibold text-gray-600 mb-1">Tidak Ada Tempat Kos</h3>
...
<div class="mt-12">
    ...
    <div class="flex items-center gap-3 mb-6">
        <div class="w-10 h-10 bg-[rgba(245,158,11,0.15)] rounded-xl flex items-center justify-center">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div x-data="{
            ...
            {{-- Badge nama kos --}}
            <div class="text-xs font-semibold text-yellow-600 bg-[rgba(245,158,11,0.08)] inline-block px-2 py-1 rounded-lg mb-3" style="flex-shrink:0;">
            ...
            <div class="ulasan-footer">
                ...
                <div>
                    <p class="text-xs font-semibold text-gray-600" style="line-height:1.2;">
...
{{-- ══════════════════════════════════════════════════
     INFO CARA BOOKING
══════════════════════════════════════════════════ --}}
<div class="mt-12 bg-[rgba(245,158,11,0.08)] border border-[rgba(245,158,11,0.3)] rounded-xl p-6">
    <div class="flex items-start">
        <svg class="w-6 h-6 text-yellow-600 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
        </svg>
        <div>
            <h3 class="font-bold text-yellow-200 mb-2">Cara Booking Kamar</h3>
